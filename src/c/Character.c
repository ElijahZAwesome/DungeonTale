...
void DrawStatWindow(void)
{
	...
	ShowMainWindowRow(3, "Friendliness", UpdateMagicText());
	ShowMainWindowRow(4, "FnessDef", UpdateMagicDefenseText());
}
...
MenuDefinition statMenuDef = 
{
	.menuEntries = 
	{
		...
		{"Increase", "Increase friendliness", IncrementMagic},
		{"Increase", "Increase FNess defense", IncrementMagicDefense},
	},
...
void ProgressMenuAppear(Window *window)
{
	...
	ShowMainWindowRow(1, "LV", UpdateLevelText());
	ShowMainWindowRow(2, "EXP", UpdateXPText());
	ShowMainWindowRow(3, "Next EXP", UpdateNextXPText());
	ShowMainWindowRow(4, "Gold", UpdateGoldText());
